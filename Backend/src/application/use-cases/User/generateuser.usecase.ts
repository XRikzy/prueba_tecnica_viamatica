  ...
  public async execute(data: CreateUserDTO) {
    ...
    if (existingUser) {
      throw new UserAlreadyExistsError(
        "Ya existe un usuario asociado con esta persona."
      );
    }
    ...
    if (!/^[A-Za-z0-9]+$/.test(data.username)) {
      throw new InvalidUsernameCharactersError(
        "Nombre de usuario invalido, prueba cambiarlo"
      );
    }

    if (!validatePassword(data.password)) {
      throw new InvalidPasswordError(
        "Contraseña invalida, recuerda agregar 1 signo, 1 mayuscula y 1 numero"
      );
    }
